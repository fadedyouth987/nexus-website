...
import type { SchedulerQueueResult } from '@/lib/automation/pipeline/types'

export type QueuePlannerToSchedulerParams = {
  userId: string
  planId: string
  orgId: string
  workspaceId: string
...
export async function queuePlannerToScheduler(
  params: QueuePlannerToSchedulerParams
): Promise<SchedulerQueueResult> {
    ...
    const publishDate =
      typeof item.publish_date === 'string' && item.publish_date
        ...
        : new Date(Date.now() + Math.max(1, Number(item.day_number || 1)) * 24 * 60 * 60 * 1000)
            ...
            .slice(0, 10)
    const platformRaw =
      typeof item.platform === 'string' && item.platform.trim() ? item.platform.trim() : 'instagram'

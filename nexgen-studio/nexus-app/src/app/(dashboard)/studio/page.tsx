...
import { ModelUploadPanel } from '@/components/generation/studio/ModelUploadPanel'
import {
  StudioGenerationControls,
  type StudioGenerationControlsRef,
} from '@/components/generation/studio/StudioGenerationControls'
...
export default function StudioPage() {
  ...
  const ensureStudioProject = useCallback(async () => {
    const response = await apiFetch('/projects')
    if (response.ok) {
      const payload = await response.json().catch(() => ({ items: [] }))
      const items = Array.isArray(payload?.items) ? payload.items : []
      const existing = items.find((item: { name?: string; id?: string }) => item.name === 'Studio Generation Pipeline')
      if (existing?.id) return existing.id as string
    }

    const createResponse = await apiFetch('/projects', {
      method: 'POST',
      body: JSON.stringify({
        name: 'Studio Generation Pipeline',
        description: 'Auto-created from Studio durable image/video submission.',
        objective: 'Bridge Studio submits onto the durable campaign and generation job model.',
        status: 'active',
      }),
    })

    if (!createResponse.ok) {
      throw new Error('Failed to create the studio project source record.')
    }
  ...
  const ensureStudioBrandKit = useCallback(async (projectId: string) => {
    const response = await apiFetch('/brand-kits')
    if (response.ok) {
      const payload = await response.json().catch(() => ({ items: [] }))
      const items = Array.isArray(payload?.items) ? payload.items : []
      const existing = items.find((item: { name?: string; id?: string }) => item.name === 'Studio Default Brand Kit')
      if (existing?.id) return existing.id as string
    }

    const createResponse = await apiFetch('/brand-kits', {
      method: 'POST',
      body: JSON.stringify({
        projectId,
        name: 'Studio Default Brand Kit',
        tone: 'Premium, cinematic, creator-first',
        palette: ['#0f172a', '#0ea5e9', '#f8fafc'],
        typography: ['Satoshi', 'Space Grotesk'],
        voiceGuidelines: 'Auto-created brand kit used to move Studio generation onto the durable source of truth.',
      }),
    })

    if (!createResponse.ok) {
      throw new Error('Failed to create the studio brand kit source record.')
    }
  ...
  const createStudioCampaign = useCallback(async (
    projectId: string,
    brandKitId: string,
    prompt: string,
    jobKind: 'image' | 'video'
  ) => {
    const campaignName = prompt.split(',')[0]?.slice(0, 80) || `Studio ${jobKind} campaign`
    const response = await apiFetch('/campaigns', {
      ...
      body: JSON.stringify({
        ...
        objective: `Studio ${jobKind} submission migrated to campaign-backed durable generation.`,
        status: 'ready',
      }),
    })

    if (!response.ok) {
      throw new Error('Failed to create campaign record for generation job.')
    }
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      if (mode === 'VIDEO' || mode === 'IMAGE') {
        const jobKind = mode === 'IMAGE' ? 'image' : 'video'
        const projectId = await ensureStudioProject()
        const brandKitId = await ensureStudioBrandKit(projectId)
        const campaignId = await createStudioCampaign(projectId, brandKitId, prompt, jobKind)

        const videoJobResponse = await apiFetch('/video-jobs', {
          method: 'POST',
          body: JSON.stringify({
            jobKind,
            projectId,
            brandKitId,
            campaignId,
            title: prompt.split(',')[0]?.slice(0, 80) || `Studio ${jobKind} job`,
            brief: prompt,
            script: mode === 'VIDEO' ? '' : undefined,
            provider: nsfwEnabled ? 'runpod' : 'comfyui',
            ...
            inputs: {
              ...
              duration_seconds: mode === 'VIDEO' ? 10 : undefined,
              aspect_ratio: outputFormat === 'story' ? '9:16' : outputFormat === 'short' ? '9:16' : '9:16',
            },
          }),
        })

        if (!videoJobResponse.ok) {
          const err = await videoJobResponse.json().catch(() => ({}))
          throw new Error((err as { detail?: string }).detail || 'Failed to submit generation job')
        }

        const createdJob = await videoJobResponse.json()
        const videoJobId = createdJob?.job?.id || createdJob?.id
        setLogLines((prev) => [...prev, `${jobKind === 'image' ? 'Image' : 'Video'} job ${videoJobId} created. Opening job detail...`])
        setGenerating(false)
        if (videoJobId) {
          router.push(`/video-jobs/${videoJobId}`)
          return
        }

        throw new Error('Durable generation job response missing id')
      }

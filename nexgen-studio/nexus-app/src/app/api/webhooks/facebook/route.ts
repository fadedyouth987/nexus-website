...
export async function POST(request: Request) {
  ...
  const result = await verifyAndStoreWebhook('facebook', rawBody, signature, headers)
  if (result.challenge) {
    return new NextResponse(result.challenge, { headers: { 'Content-Type': 'text/plain' } })
  }
  if (!result.verified) return NextResponse.json({ error: 'Invalid signature' }, { status: 401 })
